...
class CondensingSubscriber(private val loc: Location, private val topic: Topic, private val name: String) {
    private val logger = LogManager.getLogger()
...
//        setLogLevel(this.logger, Level.DEBUG)
        ...
        this.client =
            SimpleClient("localhost", 5559, identity = " CondenseSub_${topic.topic}_${loc.point}_${Random.nextInt()}")
...
//    setLogLevel(logger, Level.DEBUG)
    ...
    // prepare subscribers
    locations.forEach {
        val newS = CondensingSubscriber(it, publishTopic, "unfiltered_${i}")
        subscribers.add(newS)
        newS.prepare()

        // also prepare a subscriber for the matching topic
        val newS2 = CondensingSubscriber(it, subscriberTopic, "filtered_${i}")
