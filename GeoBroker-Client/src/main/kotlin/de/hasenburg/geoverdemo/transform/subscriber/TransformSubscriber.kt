package de.hasenburg.geoverdemo.transform.subscriber
...
class TransformSubscriber(private val loc: Location, private val topic: Topic, private val name: String) {
    private val logger = LogManager.getLogger()
    ...
    fun prepare() {
       setLogLevel(this.logger, Level.DEBUG)
        ...
        this.client =
            SimpleClient("localhost", 5559, identity = " TransformSub_${topic.topic}_${loc.point}_${Random.nextInt()}")
...
//    setLogLevel(logger, Level.DEBUG)
    ...
    val newRule = UserSpecifiedRule(locations.map { Geofence.circle(it, 2.0) }, publishTopic, File("GeoBroker-Client/src/main/kotlin/de/hasenburg/geoverdemo/transform/subscriber/jsonToCsv/"), "nodejs", subscriberTopic)


    val bridgeManager = BridgeManager()
    bridgeManager.createNewRule(newRule)

    val subscribers = mutableListOf<TransformSubscriber>()
    var i = 0

    // prepare subscribers
    locations.forEach {
        val newS = TransformSubscriber(it, publishTopic, "unfiltered_${i}")
        subscribers.add(newS)
        newS.prepare()

        // also prepare a subscriber for the matching topic
        val newS2 = TransformSubscriber(it, subscriberTopic, "filtered_${i}")
